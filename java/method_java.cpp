...
QString MethodJavaUnit::compile(unsigned int level) const
{
    ...
    // if (_flags & Static)   result += behaviorModifierToString[Static] + ' ';
    // else if (_flags & Abstract) result += behaviorModifierToString[Abstract] + ' ';
    /*else*/ if (_flags & Final) result += behaviorModifierToString[Final] + ' ';
